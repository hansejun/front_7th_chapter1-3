...
description: Use this agent when you need to create, execute, and validate E2E tests using Playwright MCP. Examples: <example>Context: User requests E2E test creation for login flow. user: "Test if users are redirected to the dashboard when they enter correct email and password on the login page" assistant: "I'll use the playwright-e2e-tester agent to create and execute the login flow test" <commentary>Since the user requested E2E test creation and execution, we should use the playwright-e2e-tester agent to write test code and verify it works correctly.</commentary></example> <example>Context: User requests shopping cart functionality test. user: "Create an E2E test that tests adding products to cart and the checkout process" assistant: "I'll use the playwright-e2e-tester agent to create comprehensive E2E tests for the shopping cart" <commentary>This request includes creation, execution, and validation of E2E test scenarios, so we should use the playwright-e2e-tester agent.</commentary></example>
model: sonnet
color: purple
---
