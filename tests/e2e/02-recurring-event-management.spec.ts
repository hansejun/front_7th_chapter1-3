...
test.describe('반복 일정 관리 E2E 테스트', () => {
  ...
  test('시나리오 1: 주간 반복 일정 생성', async ({ eventForm, eventList }) => {
    // 반복 일정 생성
    await eventForm.createRecurringEvent({
    ...
    const count = await eventList.getEventCountByTitle(recurringEventTitle);
    expect(count).toBe(4); // 주간 반복: 11/03, 11/10, 11/17, 11/24 = 4개

    // 첫 번째 이벤트가 리스트에 표시되는지 확인
    await eventList.expectEventExists(recurringEventTitle);
  });

  test('시나리오 2: 반복 일정 단일 항목 수정', async ({
    eventForm,
    eventList,
    ...
    await eventList.getEditButton(recurringEventTitle).click();

    // 반복 일정 수정 다이얼로그 처리 - "이 일정만"
    await recurringEventDialog.clickThisEventOnly();

    // 제목 수정
    const singleEditTitle = `${recurringEventTitle} (단일 수정)`;
    await eventForm.titleInput.fill(singleEditTitle);

    // 위치 수정
    await eventForm.locationInput.fill('회의실 B');

    // 일정 수정 버튼 클릭
    await eventForm.submitButton.click();

    // 폼이 리셋될 때까지 대기
    await eventForm.waitForFormReset();

    // UI를 통해 수정된 이벤트 확인
    const singleEditCount = await eventList.getEventCountByTitle(singleEditTitle);
    ...
    const unchangedCount = await eventList.getEventCountByTitle(recurringEventTitle);
    expect(unchangedCount).toBe(3); // 4개 중 1개 수정, 3개 유지
  });

  test('시나리오 3: 반복 일정 전체 시리즈 수정', async ({
    eventForm,
    eventList,
    recurringEventDialog,
  }) => {
    ...
    await recurringEventDialog.clickAllEvents();

    // 설명 수정
    const seriesEditDescription = `전체 시리즈 수정됨 (${recurringEventTimestamp})`;
    await eventForm.descriptionInput.fill(seriesEditDescription);

    // 위치 수정
    await eventForm.locationInput.fill('회의실 C');
  ...
  test('시나리오 4: 반복 일정 단일 항목 삭제', async ({
    page,
    eventForm,
    eventList,
    recurringEventDialog,
    ...
    await eventList.getDeleteButton(recurringEventTitle).click();
  ...
  test('시나리오 5: 반복 일정 전체 시리즈 삭제', async ({
    page,
    eventForm,
    eventList,
    recurringEventDialog,
    ...
    await eventList.expectEventNotExists(recurringEventTitle);
  });

  test('시나리오 6: 다양한 반복 유형 테스트 - 매일', async ({ eventForm, eventList }) => {
    ...
    // UI를 통해 생성된 일정 개수 확인
    const count = await eventList.getEventCountByTitle(dailyTitle);
    expect(count).toBe(5); // 매일 반복: 11/03 ~ 11/07 = 5개
  });

  test('시나리오 6: 다양한 반복 유형 테스트 - 매월', async ({ page, eventForm, eventList }) => {
    ...
    // 11월에 1개 확인
    const novemberCount = await eventList.getEventCountByTitle(monthlyTitle);
    ...
    // 12월에 1개 확인
    const decemberCount = await eventList.getEventCountByTitle(monthlyTitle);
    expect(decemberCount).toBe(1); // 12월 1일
  });

  test('시나리오 6: 다양한 반복 유형 테스트 - 매년', async ({ eventForm, eventList }) => {
    ...
    // UI를 통해 생성된 일정 개수 확인
    const count = await eventList.getEventCountByTitle(yearlyTitle);
